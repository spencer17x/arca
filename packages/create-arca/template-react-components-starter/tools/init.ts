...
const renameFiles = [
  ...
  ["tests/library.test.ts", "tests/--libraryname--.test.ts"]
]
...
function modifyContents(libraryName: string, username: string, usermail: string) {
  ...
  const splitLibraryName = libraryName.split("-")
  const UpperCamelCase = splitLibraryName.map(
    word => upperCaseFirstLetter(word)
  ).join('');
  const lowerCamelCase = splitLibraryName.map(
    (word, index) => index > 0 ? upperCaseFirstLetter(word) : word
  ).join('')
  try {
    const changes = replace.sync({
      files,
      from: [/--libraryname--/g, /--username--/g, /--usermail--/g, /--UpperCamelCase--/g, /--lowerCamelCase--/g],
      to: [libraryName, username, usermail, UpperCamelCase, lowerCamelCase]
    })
